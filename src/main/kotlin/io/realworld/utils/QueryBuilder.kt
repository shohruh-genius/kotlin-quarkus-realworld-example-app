...
internal class QueryBuilder(
    ...
    internal fun add(clause: Clause): QueryBuilder =
        this.apply {
            when (clause) {
                ...
                is WHERE -> {
                    if (whereStatements.size > 0) {
                        andStatements.add(AND(clause.value))
                    } else {
                        whereStatements.add(clause)
                    }
                }
    ...
    internal fun add(vararg clauses: Clause): QueryBuilder =
        this.apply {
            clauses.forEach(::add)
        }
    ...
    internal fun build(): String =
        StringBuilder().run {
            check(
                selectStatements.isNotEmpty()
            ) { "Query builder expects at least one SELECT query (i.e. QueryBuilder.query(SELECT(\"*\"))" }
            append(
                sequenceOf<Clause>()
